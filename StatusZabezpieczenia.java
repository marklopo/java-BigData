...
public class StatusZabezpieczenia implements IStatusO {
	public void status() {
		String obiekt = pobierzObiekt();
		String status = pobierzStatus();
		try {
			String query = "select * from zabezpieczenia where Obiekt=? and Status=?";
			...
			if (set.next() == true) {
				...
				try {
					String query2 = "update zabezpieczenia set Status=? where Obiekt=?";
